...
function getLevel(areaDivId,levelName,id1,levelName2,id2){
	var cit = "<option value=''>全部</option>";   
	var areaId=$("#"+areaDivId).val();
	if($.trim(areaId)==""){
		areaId="00";
	}
	var levelVal=$("#"+id1).val();
	if($.trim(levelVal)==""){
		$('#'+id2).empty().append(cit);  
	}else{
		$.ajax({
			async:false,
			type: "post",
			url: "/bestv/report/product/getLevel",
			data: "areaId="+areaId+"&levelName="+levelName+"&level="+levelVal+"&level02="+levelName2,
			success: function(msg){
				var data = $.evalJSON(msg);
				$.each(data, function(i, n){
	               			 cit+="<option value="+n+">"+n+"</option>" ;
